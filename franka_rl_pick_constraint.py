...
class FrankaPickEnv(gym.Env):
    ...
    def __init__(self, render=False, max_steps=150):
        ...
        self.workspace_low = np.array([-0.1, -0.3, 0.4])
        self.workspace_high = np.array([0.6, 0.3, 1.0])

        # --- grasp states ---
        self.grasp_constraint_id = None
        self.is_grasped = False
        self.grasp_dist_thresh = 0.05
        self.grasp_width_thresh = 0.015
    ...
    def _setup_world(self):
        ...
        p.loadURDF("table/table.urdf", [0, 0, 0], True, physicsClientId=self.client_id)

        cx = 0.25 + 0.1 * np.random.uniform(-1, 1)
        cy = 0.05 * np.random.uniform(-1, 1)
        self.cube_id = p.loadURDF("cube_small.urdf", [cx, cy, 0.65], physicsClientId=self.client_id)
    ...
    def _get_ee_pos(self):
        return np.array(p.getLinkState(self.robot_id, self.EE_LINK, True)[0])

    def _get_ee_orn(self):
        return p.getLinkState(self.robot_id, self.EE_LINK, True)[1]

    def _get_cube_pos(self):
        return np.array(p.getBasePositionAndOrientation(self.cube_id)[0])

    def _get_obs(self):
        ee = self._get_ee_pos()
        cube = self._get_cube_pos()
        diff = cube - ee
        grip = p.getJointState(self.robot_id, self.FINGER_JOINTS[0])[0]
    ...
    def _try_grasp(self):
        ...
        # 🔧 放寬距離 & 開口門檻
        if dist < 0.06 and self.grip_width < 0.025:
    ...
    def step(self, action):
        ...
        self.grip_width = float(np.clip(self.grip_width + dg * self.grip_scale, 0, 0.04))
        ...
        for j in self.ARM_JOINTS:
            p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, joints[j], force=90)

        for j in self.FINGER_JOINTS:
            p.setJointMotorControl2(self.robot_id, j, p.POSITION_CONTROL, self.grip_width, force=40)
        ...
        ee = self._get_ee_pos()
        cube = self._get_cube_pos()
        dist = np.linalg.norm(cube - ee)

        # --------- 基本 shaping ---------
        reward = -dist - 0.01   # 距離越近越好

        # --------- Curriculum: 先逼近、再關爪 ---------

        # ① 接近階段：只要在 20cm 以內，多給一點穩定的接近獎勵
        if dist < 0.20:
            reward += 0.2

        # ② 抓取階段：在非常靠近 (8cm 以內) 的時候，
        #    開口越小 → 額外獎勵越大，逼它學會「碰到就關」
        if dist < 0.1:
            # grip_width ∈ [0, 0.04]，我們把「越小」線性轉成 bonus
            close_bonus = (0.04 - self.grip_width) / 0.04   # 0~1
            reward += 10.0 * close_bonus                      # 權重可再調

        # ③ 一旦成功 grasp（有 constraint）給明顯 bonus
        if self.is_grasped:
            reward += 50.0

        # ④ 已經抓住，再把 cube 抬高 → 成功
        terminated = False
        if self.is_grasped and cube[2] > 0.75:
            reward += 100.0
            terminated = True
